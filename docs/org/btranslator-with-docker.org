...
* Getting images 
  ...
  docker pull btranslator/btr_client:v2.1
  docker pull btranslator/btr_client:v2.1-openatrium
  docker pull btranslator/btr_server:v2.1
  docker images
  #+END_EXAMPLE

  Docker images can be deleted with the command =docker rmi=:
  #+BEGIN_EXAMPLE
  docker rmi btranslator/btr_client:v2.1 btranslator/btr_server:v2.1
  #+END_EXAMPLE
...
* Creating containers
  ...
  docker run -d --name=bcl --hostname=example.org \
             -p 80:80 -p 443:443 btranslator/btr_client:v2.1
  #+END_EXAMPLE
  ...
  #+BEGIN_EXAMPLE
  docker run -d --name=bcl1 -p 81:80 -p 444:443 btranslator/btr_client:v2.1
  docker run -d --name=bcl2 -p 82:80 -p 445:443 btranslator/btr_client:v2.1
  #+END_EXAMPLE
